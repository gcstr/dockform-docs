...
| **apply** | Lists existing labeled volumes and creates any missing ones from `volumes:` and `filesets.target_volume`.<br>Labels volumes with `io.dockform.identifier=<docker.identifier>`.<br>Proceeds to sync filesets and run `docker compose up` for stacks. |
